...
// joi - 할 일 변경 로직
// 1. 'value' 데이터는 필수적으로 존재 하여야한다.
// 2. 'value' 데이터는 문자열 타입 이여야한다.
// 3. value 데이터는 최소 1글자 이상 이여야한다.
// 4. value 데이터는 최대 50글자 이하 이여야한다.
// 5. 유효성 검사에 실패했을 때, 에러가 발생하여야한다.
...
router.post("/todos", async (req, res, next) => {
  ...
  return res.status(201).json({ todo, order });
});
...
// 해야할 일 순서 변경 api, 완료, 해제 추가 , 내용 변경 추가로 추가
...
router.delete("/todos/:todoId", async (req, res, next) => {
  ...
  if (!todo) {
    return res
      .status(404)
      .json({ errorMessege: "존재하지 않는 해야할 일 정보입니다." });
  }
